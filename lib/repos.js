...
Repo.prototype._resetCache = function() {
  this.icache = null;
}
...
Repo.prototype.saveIssue = function (i) {
  ...
  var oifn = this.issuePath(i.issue_id);
...
Repo.prototype.archiveIssue = function (issue_id) {
  ...
  const ifn = this.issueFileName(issue_id);
  ...
  try {
    fs.renameSync(path.join(this.ipath, ifn), path.join(ap, ifn));
    this._resetCache();
    return true;
  } catch (e) {
...
Repo.prototype.issueFileName = function(issue_id) {
...
Repo.prototype.issuePath = function (issue_id) {
  var ifn = this.issueFileName(issue_id);

  return ifn ? path.resolve(path.join(this.ipath, ifn)) : null;
}
