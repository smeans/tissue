...
Repo.prototype.list = function () {
  return fs.readdirSync(this.ipath).filter(n => path.extname(n) == '.md').map((v) => v.substring(0, v.length-'.md'.length));
}
...
Repo.prototype.issuePath = function (issue_key) {
  return path.resolve(path.join(this.ipath, issue_key + '.md'));
}
